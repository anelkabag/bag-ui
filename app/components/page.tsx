...
function CliButton({ item }: { item: RegistryItem }) {
  ...
  return (
    <div className="relative flex-shrink-0" onMouseLeave={() => setOpen(false)}>
      <div className="flex items-center gap-1.5 rounded-full border border-border bg-background pl-3 pr-1.5 py-1.5 text-xs font-mono text-muted-foreground hover:border-muted-foreground/40 transition-colors ">
        <button
          onClick={handleCopy}
        ...
        <button
          ...
          className="flex items-center gap-1 pl-1.5 border-l border-border hover:text-foreground transition-colors cursor-pointer"
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-20 min-w-[110px] rounded-lg border border-border bg-popover shadow-lg overflow-hidden">
          {PKG_MANAGERS.map((p) => (
            <button
              key={p}
              onClick={(e) => {
...
function AccessBadge({ tier }: { tier: AccessTier }) {
  ...
  return (
    <span className="absolute top-2 right-2 z-10 text-[10px] font-medium px-2 py-0.5 rounded-full bg-foreground text-background">
      Pro
    </span>
  );
}
...
function ComponentCard({ item }: { item: RegistryItem }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-3 sm:p-4 transition-shadow hover:shadow-md">
