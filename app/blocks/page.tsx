...
const AVAILABLE_PREVIEW_COMPONENTS: Record<
  string,
  React.ComponentType<unknown>
> = {
  hero: dynamic(
    () =>
      import("../../registry/default/blocks/hero/hero1").then(
        (mod) => mod.default,
      ),
    { ssr: false },
  ),
  team: dynamic(
    () =>
      import("../../registry/default/blocks/teams/team-section").then(
        (mod) => mod.default,
      ),
    { ssr: false },
  ),
...
export function BlockPreview({ id }: { id: string }) {
  const PreviewComponent = AVAILABLE_PREVIEW_COMPONENTS[id];
...
function BlockCard({
  ...
  return (
    <Link href={`/blocks/${id}`} className="group block">
      ...
      <div className="relative overflow-hidden rounded-xl border border-gray-200 bg-gray-50 aspect-video mb-3 transition-all duration-200 group-hover:border-gray-400 group-hover:shadow-md">
        <div className="absolute inset-0">
          <BlockPreview id={id} />
        </div>
        {/* overlay on hover */}
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/3 transition-colors duration-200" />
        {/* badges */}
