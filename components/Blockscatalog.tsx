                ...
                {[0, 1, 2].map((i) => (
                    <g key={i} transform={`translate(${84 + i * 60},26)`}>
                        <circle
                            ...
                            cy="8"
                            r="8"
                            fill={i === 0 ? "#111827" : i === 1 ? "#d1d5db" : "#f3f4f6"}
...
function BlockCard({ block, index }: { block: SectionBlock; index: number }) {
    const cardContent = (
        <Link href={block.available && block.route ? `/blocks/${block.route}` : "#"} className="group block">
            {/* Preview thumbnail */}
            <div className="relative overflow-hidden rounded-lg border border-gray-200 bg-gray-50 aspect-[16/9] mb-2 transition-all duration-200 group-hover:border-gray-400 group-hover:shadow-md">
                {/* SVG preview */}
